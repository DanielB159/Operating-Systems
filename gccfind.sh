...
if [ $# -eq 2 ]; then
    ...
    find $1  -maxdepth 1 -name "*.c" -type f -print0 | xargs -0 grep -l --null "$2" | xargs -0 sh -c 'for file do gcc -o "${file%.c}".out -w "$file"; done' sh 
...
if [ $# -eq 3 ]; then
    ...
    find $1 -name "*.c" -type f -print0 | xargs -0 grep -l --null "$2" | xargs -0 sh -c 'for file do gcc -o "${file%.c}".out -w "$file"; done' sh 
fi
